...
def command_out(command):
    process = subprocess.Popen(command, shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE)  # Use shell=False for safety
...
def mitre_lab_17_api(request):
    if request.method == "POST":
        ...
        if not re.match(r'^((25[0-5]|2[0-4]\d|[0-1]?\d?\d)(\.|$)){4}$', ip):
            return JsonResponse({'error': 'Invalid IP address'}, status=400)  # TODO: Replace placeholder error message as needed
        command = ['nmap', ip]  # Use list to avoid shell injection vulnerabilities 
